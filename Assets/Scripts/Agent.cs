...
public class Agent : MonoBehaviour
{
    ...
    Vector3 cohesionB()
    {
        // cohesion behavior:  creating Vectors of Radius(rads) used in Cohesion()

        Vector3 rads = new Vector3();
        ...
        if (neighbours.Count == 0)                   // check if there is neighbours in vector
            return rads;                             // if no neighbours identified return as 0 Vector
        ...
        foreach (var agent in neighbours)    // use Iteration to find out the COM(center of mass) of neighbours
        {
            ...
            {
                rads += agent.x;           // add position of agents
                countinAgents++;
            }
        }
        ...
        rads /= countinAgents;         // divide the number of agents identified 

        // a vector from our position x towards the COM r
        rads = rads - this.x;

        rads = Vector3.Normalize(rads);   // Give a result of vector with magnitude of 1
    ...
    Vector3 separationB()
    {
        ...
        if (agents.Count == 0)
            return rads;    // if no neighbours return as 0 vector.
        ...
        foreach (var agent in agents)   // Iterate each of our neighbours
        {
            if (withinFieldOfView(agent.x))
            {
                ...
                {
                    rads += towardsMe.normalized / towardsMe.magnitude;  // Add to the resulting radius which is the scaled distance; will not make it proportionate to the square of the distance , just the regular distance
                }
            }
        }

        return rads.normalized;     // return the normalizex Vector
    }
    ...
    Vector3 alignmentB()
    {
        ...
        foreach (var agent in agents)   // Iterate all our neighboring agents
            if (withinFieldOfView(agent.x))
                rads += agent.vel;   // Add their velocity to the radius of neighbour's radius

        return rads.normalized;
    }
    ...
    virtual protected Vector3 combineB()
    {
        // combine all desires
        // weighted sum
        Vector3 rads = config.KfAvoid * avoidAgentEnemies()
                        + config.KfC * cohesionB()
                        + config.KfS * separationB()
                        + config.KfA * alignmentB()
                        + config.KfW * wanderB();
        // end results of all will be stored in rads
        return rads;
    }
    ...
    Vector3 wanderTrgt;      // wanderTarget position
    ...
    protected Vector3 wanderB()
    {
        ...
        // Just to test the Wander Behavior :    debugWanderCube.transform.position = targetInWorldArea;
    ...
    Vector3 avoidAgentEnemies()
    {
        ...
        {
            rads += flee(enemy.x);   // enemy inherited from agent , does have an x position
        }
        return rads.normalized;
    }
